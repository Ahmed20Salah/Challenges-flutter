...
class AllTeamsWidget extends StatelessWidget {
                  ...
                  children: [
                   ...
                   switch(logic.rxRequestStatusAllTeam.value){
                     ...
                     RequestStatus.SUCESS => ListView.separated(
                         physics: NeverScrollableScrollPhysics(),
                         shrinkWrap: true,
                         itemCount: logic.rxAllTeamModel?.length??0,
                         itemBuilder: (context, index) {
                           return ListTile(

                               leading: CircleAvatar(
                                 radius: 20,
                                 foregroundImage: CachedNetworkImageProvider(logic.rxAllTeamModel?[index].image != null  ?API.imageUrl(logic.rxAllTeamModel?[index].image??''):'https://cdn.logojoy.com/wp-content/uploads/2018/05/30161703/251.png'),

                               ),
                               title: Text(logic.rxAllTeamModel?[index].name ?? '',
                                 style: getBoldItalicStyle(
                                     color: ColorManager
                                         .blackText,
                                     fontSize: FontSize.s16
                               ...
                               trailing: Radio<TeamModel>(
                                 value: logic.rxAllTeamModel?[index]??TeamModel(),
                                 groupValue: logic.selectedTeam,
                                 onChanged: (TeamModel? value) {
                                   logic.onChangeRadioTeam(value);

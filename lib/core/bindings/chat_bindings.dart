...
class ChatBindings implements Bindings{
  ...
  void dependencies() {
    Get.lazyPut<IChatDataSource>(
          () => ChatDataSource(
          Get.find<IConnectivityChecker>(), Get.find<WebServiceConnections>()),fenix: true
    );
 Get.lazyPut(() => ChatController( Get.find<IChatDataSource>(),Get.find<IImagePicker>()),fenix: true);
  }
